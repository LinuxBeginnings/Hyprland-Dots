...
wallust_prepare_args() {
  ...
  if [ -n "$major" ] && [ "$major" -ge 4 ]; then
    local v4_cfg="${XDG_CONFIG_HOME:-$HOME/.config}/hypr/wallust/wallust-v4.toml"
    local v4_kitty_cfg="${XDG_CONFIG_HOME:-$HOME/.config}/hypr/wallust/wallust-kitty-v4.toml"
